...
router.post("/", requireAuth, async (req, res) => {
  ...
  const cart_products = await Promise.all(
    cartItems.map(async (cartItem) => {
      ...
      const deliveryOption = await db
        .select()
        .from(deliveryOptions)
        .where(eq(deliveryOptions.id, cartItem.deliveryOptionId));
      if (product.length === 0 || deliveryOption.length === 0) {
        return res
          .status(400)
          .json({ error: "Invalid product or delivery option" });
      }
      ...
      const estimatedDeliveryTimeMs =
        Date.now() + deliveryOption[0].deliveryDays * 24 * 60 * 60 * 1000;
      return {
...
router.get("/:orderId", requireAuth, async (req, res) => {
  ...
  let orderRow = await db
    .select()
    .from(orders)
    .where(and(eq(orders.id, orderId), eq(orders.userId, userId)));
  console.log("tracking", userId, orderId, orderRow);
